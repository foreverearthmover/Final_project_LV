/***
  e-Angel
  ---------------------------------
  Interactive Tamagotchi-style angel using an OLED display,
  three buttons, a tilt switch, and a buzzer.

  The angel reacts to user interaction:
   - ASK  : Ask the angel a question (requires shaking device)
   - PRAY : Makes the angel polite and more positive
   - PET  : Pet the angel; repeated pets give a blessing
 
  If the angel is ignored for too long it becomes annoyed,
  and eventually transforms into a demon.

  Buttons:
   ASK  -> D2
   PRAY -> D3
   PET  -> D4

  Buzzer:
   D5
 
  Tilt Switch:
   D12
 
  Libraries used:
   - Adafruit SSD1306
   - Adafruit GFX
   - CuteBuzzerSounds
***/

#include <Wire.h>
#include <Adafruit_GFX.h>
#include <Adafruit_SSD1306.h>
#include "bitmaps.h" // contains angel bitmaps
#include <CuteBuzzerSounds.h> // buzzer sounds

// OLED DISPLAY CONFIGURATION
#define SCREEN_WIDTH 128
#define SCREEN_HEIGHT 64

Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire);

// BUTTON PINS
const int buttonAsk = 2;
const int buttonPray = 3;
const int buttonPet = 4;

// BUZZER PIN
const int buzzerPin= 5;

// TILT SWITCH
const int tiltPin = 12; 

int tiltState = HIGH;
int lastTiltState = HIGH;

// ASK + SHAKE logic
bool waitingForShake = false;
bool showingAskPrompt = false;

unsigned long askPromptStart = 0;
const unsigned long ASK_PROMPT_DURATION = 4000; // prompt visible for 4 seconds

// INTERACTION MODES
enum InteractionMode {
  MODE_IDLE,
  MODE_WAIT_SHAKE,
  MODE_SHOW_ANSWER,
  MODE_SHOW_STATUS
};

InteractionMode mode = MODE_IDLE;

unsigned long modeStartTime = 0;
unsigned long lastInteractionTime = 0;

// time threshold before the angel becomes upset
const unsigned long ANNOY_TIME = 20000;   // 20s 
const unsigned long DEMON_TIME = 40000;   // 40s 

//STATES
enum AngelState {
  ANGEL_NEUTRAL,
  ANGEL_POLITE,
  ANGEL_ANNOYED,
  ANGEL_SURPRISED,
  ANGEL_DEMON
};

AngelState angelState = ANGEL_NEUTRAL;
AngelState previousState = ANGEL_NEUTRAL;

// FLOATING ANIMATION 
int angelBaseY = 0;      // center Y for fullscreen sprite 
int angelOffsetY = 0;   // offset for flying animation
int floatDirection = 1;

unsigned long lastFloatUpdate = 0;
const unsigned long floatInterval = 120; // animation speed (ms)

// PRAYER TIMER
unsigned long lastPrayTime = 0;
const unsigned long PRAY_TIMEOUT = 15000; // 15 seconds

// ANGEL NUMBERS
int angelNumbers[] = {111, 222, 333, 444, 555, 777, 888, 999};
const int ANGEL_COUNT = 8;

// ASK ANSWERS
const char* neutralAnswers[] = {
  "Maybe.",
  "Hard to say.",
  "The signs are unclear.",
  "Ask again later."
};

const char* prayedAnswers[] = {  
  "Yes. Trust it.",
  "The universe agrees.",
  "No, be careful.",
  "It will unfold soon."
};

const char* rudeAnswers[] = {
  "You already know.",
  "Why ask me?",
  "Whatever.",
  "Stop bothering me."
};

// PET COUNTER
int petCounter = 0;
const int PET_THRESHOLD = 3;

// SETUP 
void setup() {
  // configure input pins
  pinMode(buttonAsk, INPUT_PULLUP);
  pinMode(buttonPray, INPUT_PULLUP);
  pinMode(buttonPet, INPUT_PULLUP);
  pinMode(tiltPin, INPUT_PULLUP);

  // initialize buzzer
  cute.init(buzzerPin);
  cute.play(S_CONNECTION); // start up sound

  lastInteractionTime = millis();

  // initialize OLED display
  display.begin(SSD1306_SWITCHCAPVCC, 0x3C);
  display.clearDisplay();

  drawAngel();
  display.display();
}

// MAIN LOOP
void loop() {

  checkButtons();
  checkTilt();
  updateState();
  updateAngelFloat();

  // MODE HANDLING
  // idle mode: simply displays angel
  if (mode == MODE_IDLE) {
   display.clearDisplay();
    drawAngel();
    display.display();
  }
    
  //waiting for user to shake device
  else if (mode == MODE_WAIT_SHAKE) {

    display.clearDisplay();
    drawAngel();   // surprised angel animates

    // draws prompt text
    display.setTextSize(1);
    display.setTextColor(WHITE);
    display.setCursor(8, 54);
    display.print("Shake for answers");
    display.display();

    // timeout: cancel asking if no shake
    if (millis() - modeStartTime > 5000) {
      mode = MODE_IDLE;
      angelState = ANGEL_NEUTRAL;
      waitingForShake = false;
    }
  }
    
  //display answer temporarily
  else if (mode == MODE_SHOW_ANSWER) {

    // visible for 3 sec
    if (millis() - modeStartTime > 3000) {
      mode = MODE_IDLE;
      angelState = ANGEL_NEUTRAL;
    }
  }

  // display status message
  else if (mode == MODE_SHOW_STATUS) {

    // visible for 3 sec
    if (millis() - modeStartTime > 3000) {
     mode = MODE_IDLE;
    }
  }
}

// BUTTON HANDLING
void checkButtons() {

  // ASK BUTTON
  if (digitalRead(buttonAsk) == LOW && !waitingForShake) {

    if (angelState == ANGEL_DEMON) { 
      showStatusMessage("You're a fool.");
      delay(250);
      return;
    }

    waitingForShake = true;
    showingAskPrompt = true;
    askPromptStart = millis();

    previousState = angelState;
    angelState = ANGEL_SURPRISED;

    
    mode = MODE_WAIT_SHAKE;
    modeStartTime = millis();

    delay(250); 

  // timeout condition
  } else if (digitalRead(buttonAsk) == HIGH && waitingForShake && !showingAskPrompt) {
    // button was released and we were waiting for a shake
    // but the prompt is no longer showing (meaning it timed out or an answer was shown) → reset waiting for shake
    waitingForShake = false;
  }

  // PRAY BUTTON 
  if (digitalRead(buttonPray) == LOW) { 

    if (angelState == ANGEL_DEMON) { 
      showStatusMessage("I don't care.");
      delay(250);
      return;
    }

    angelState = ANGEL_POLITE; 
    lastPrayTime = millis(); 
    lastInteractionTime = millis(); 
    petCounter = 0; 
    drawAngel(); 
    showStatusMessage("Prayer received."); 
    delay(250); 
  }

// PET BUTTON
if (digitalRead(buttonPet) == LOW) {
  
  if (angelState == ANGEL_DEMON) {
  showStatusMessage("Don't even try.");
  delay(250);
  return;
  }

  lastInteractionTime = millis();
  petCounter++;

  if (petCounter == 1) {
    showStatusMessage("Do that again!");
  }
  else if (petCounter == 2) {
    showStatusMessage("One more time...");
  }
  else if (petCounter >= PET_THRESHOLD) {

    // reset first to avoid retrigger issues
    petCounter = 0;

    drawAngelNumber();        // show blessing
    delay(2000);
    showStatusMessage("You are blessed.");
  }
  delay(250); 
}

}

// STATE MANAGEMENT
void updateState() {

  unsigned long idleTime = millis() - lastInteractionTime;

  // angel turns annoyed
  if (angelState != ANGEL_DEMON &&
      idleTime > ANNOY_TIME &&
      angelState != ANGEL_ANNOYED) {

    angelState = ANGEL_ANNOYED;
    showStatusMessage("I feel lonely.");
    cute.play(S_OHOOH); // annoyed sound
  }

  // turns demonic
  if (idleTime > DEMON_TIME &&
      angelState != ANGEL_DEMON) {

    angelState = ANGEL_DEMON;
    showStatusMessage("You ignored me...");
    cute.play(S_OHOOH2); // angry sound
  }
}

// FLOATING ANIMATION
void updateAngelFloat() {
  if (millis() - lastFloatUpdate > floatInterval) {
    lastFloatUpdate = millis();

    angelOffsetY += floatDirection;

    if (angelOffsetY >= 1 || angelOffsetY <= -1) {
      floatDirection *= -1;
    }
  }
}


// TILT / SHAKE DETECTION
void checkTilt() {
  
  tiltState = digitalRead(tiltPin);

  if (mode == MODE_WAIT_SHAKE &&
      lastTiltState == HIGH &&
      tiltState == LOW) {

    showingAskPrompt = false;
    angelState = previousState;
    handleAsk();               // show answer
  }

  lastTiltState = tiltState;
}

// ASK / ANSWER LOGIC
void handleAsk() {

  mode = MODE_SHOW_ANSWER;
  modeStartTime = millis();

  display.clearDisplay();
  cute.play(S_BUTTON_PUSHED);

  // draw angel (not surprised anymore)
  drawAngel();

  display.setTextSize(1);
  display.setTextColor(WHITE);
  display.setCursor(10, 55);

  if (angelState == ANGEL_POLITE) {
    display.print(prayedAnswers[random(0,4)]);
  }
  else if (angelState == ANGEL_ANNOYED) {
    display.print(rudeAnswers[random(0,4)]);
  }
  else {
    display.print(neutralAnswers[random(0,4)]);
  }
  display.display();
}

// DRAW ANGEL SPRITE
void drawAngel() {

  int y = angelBaseY + angelOffsetY;

  // draw different faces
  switch (angelState) {
    case ANGEL_NEUTRAL:
      display.drawBitmap(0, y, neutralAngel, 128, 64, SSD1306_WHITE);
      break;

    case ANGEL_POLITE:
      display.drawBitmap(0, y, politeAngel, 128, 64, SSD1306_WHITE);
      break;

    case ANGEL_ANNOYED:
      display.drawBitmap(0, y, annoyedAngel, 128, 64, SSD1306_WHITE);
      break;

    case ANGEL_SURPRISED:
      display.drawBitmap(0, y, surprisedAngel, 128, 64, SSD1306_WHITE);
      break;
    case ANGEL_DEMON:
      display.drawBitmap(0, y, angryDemon, 128, 64, SSD1306_WHITE);
     break;
  }
}

// DRAW ANGEL NUMBER
void drawAngelNumber() {
  display.clearDisplay();
  display.fillRect(0, 0, 128, 64, BLACK); // prevents stray pixels from showing up

  int index = random(0, ANGEL_COUNT);
  int number = angelNumbers[index];

  display.setTextSize(1);
  display.setTextColor(WHITE);
  display.setCursor(20, 10);
  display.print("You received an");

  display.setCursor(20, 22);
  display.print("angel number:");

  display.setTextSize(2);
  display.setCursor(40, 40);
  display.print(number);
  
  soundEffect(buzzerPin);

  display.display();

  // Reset after blessing
  petCounter = 0;
}

// DISPLAY STATUS MESSAGE
void showStatusMessage(const char* msg) {
  
  mode = MODE_SHOW_STATUS;
  modeStartTime = millis();

  display.clearDisplay();
  drawAngel();

  display.setTextSize(1);
  display.setTextColor(WHITE);
  display.setCursor(8, 54);
  display.print(msg);

  display.display();
}

// ANGEL NUMBER SOUND EFFECT
void soundEffect(int buzzer)  {

  #define NOTE_C4  262
  #define NOTE_E4  330
  #define NOTE_F4  349
  #define NOTE_C5  523
  #define REST      0

  // adjustable tempo
  int tempo = 200;

  int melody[] = {
    NOTE_F4,8, 
    NOTE_E4,8, 
    NOTE_C4,4, 
    NOTE_F4,4,
    NOTE_C5,-2, 
  };

  // sizeof gives the number of bytes, each int value is composed of two bytes (16 bits)
  // there are two values per note (pitch and duration), so for each note there are four bytes
  int notes = sizeof(melody) / sizeof(melody[0]) / 2;

  // calculates the duration of a whole note in ms
  int wholenote = (60000 * 4) / tempo;
  int divider = 0, noteDuration = 0;

  // iterate over the notes of the melody.
  // array is twice the number of notes (notes + durations)
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {

    // calculates the duration of each note
    divider = melody[thisNote + 1];
    if (divider > 0) {
      // regular note, just proceed
      noteDuration = (wholenote) / divider;
    } else if (divider < 0) {
      // dotted notes are represented with negative durations
      noteDuration = (wholenote) / abs(divider);
      noteDuration *= 1.5; // increases the duration in half for dotted notes
    }

    // we only play the note for 90% of the duration, leaving 10% as a pause
    tone(buzzer, melody[thisNote], noteDuration*0.9);

    // wait for the specific duration before playing the next note.
    delay(noteDuration);
    
    // stop the waveform generation before the next note.
    noTone(buzzer);
  }
}
